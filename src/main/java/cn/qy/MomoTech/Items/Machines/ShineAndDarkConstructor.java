...
public class ShineAndDarkConstructor extends AbstractGUI implements RecipeDisplayItem {
        ...
        for (int i : getInputSlots()) {
            ...
            if (it.equals(new ItemStack(Material.NETHERITE_PICKAXE))) {
                ...
                if (Maths.GetRandom(100) == 0) {
                    if (k < 0) {
                        inv.pushItem(MomotechItem.dark, getOutputSlots());
                        return;
                    } else if (k > 256) {
                        inv.pushItem(MomotechItem.shine, getOutputSlots());
                        return;
                    }
                } else {
                    inv.pushItem(MomotechItem.empty_shell, getOutputSlots());
                    return;
                }
            }
        }
